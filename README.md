...
Chunk size and max roller (below) are other configs specifying how many columns $\mathcal{K}$ will have. Rollout mulitple multiplies the number of rows of $\mathcal{K}$, which we found to be unnecessary. Other self-evident parameters are epochs and batch size. 
